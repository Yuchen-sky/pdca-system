...
public class PlanPostController {
    ...
    @LoginRequired
    @RequestMapping(path="/setfinishbyid/{planid}",method = RequestMethod.GET)
    public String setfinishbyid(
            Model model, @PathVariable("planid") int planid
    ){
        planPostService.updateFinish(planid,1);
    ...
    @LoginRequired
    @RequestMapping(path="/setdelaybyid/{planid}",method = RequestMethod.GET)
